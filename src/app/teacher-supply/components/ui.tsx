...
export function ProblemCard({
  ...
  return (
    <article className="lingoace-panel-hover overflow-hidden rounded-2xl border border-slate-100 bg-white">
      <div className="border-b border-slate-100 bg-slate-50/60 px-5 py-4">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="font-bold text-slate-900">{title}</h3>
          <LabelChip tone={severity === "高" ? "pain" : "warn"}>
            风险 · {severity}
          </LabelChip>
        </div>
      </div>
      <div className="grid gap-0 md:grid-cols-3">
        <div className="border-b border-slate-100 p-5 md:border-b-0 md:border-r">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-rose-600">
            问题
          </p>
        ...
        <div className="border-b border-slate-100 p-5 md:border-b-0 md:border-r">
          ...
          <p className="text-sm leading-relaxed text-slate-700">{thinking}</p>
        </div>
        <div className="p-5">
          ...
          <p className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
            <span className="font-semibold text-slate-600">验收：</span>
            {verify}
          </p>
        </div>
      </div>
